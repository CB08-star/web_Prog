...
<?php
	...
	$que1=mysqli_query($con,"select * from user_secret_quotes where user_id=$userid and Answer2='$ans2'");
	$count1=mysqli_num_rows($que1);
	
	if($count1>0)
	{
		$que2=mysqli_query($con,"select * from users where user_id=$userid");
		$rec1=mysqli_fetch_row($que2);
		$password=$rec1[3];
?>
<?php
