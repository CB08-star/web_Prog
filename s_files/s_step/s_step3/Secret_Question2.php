<?php
	session_start();
	error_reporting(1);
	if(isset($_SESSION['tempsuser']))
	{
		$con= mysqli_connect("localhost","root","");
		mysqli_select_db($con,"sdb");
		$user=$_SESSION['tempsuser'];
		...
		if($q2=="" && $a2=="")
		{
			$que3=mysqli_query($con,"select * from user_secret_quotes where user_id=$userid");
...
<?php
	if(isset($_POST['Finish']))
	{
		...
		mysqli_query($con,"update user_secret_quotes set Question2='$que2',Answer2='$ans2' where user_id=$userid");
		
		$que_user_data=mysqli_query($con,"select * from users where Email='$user';");
		$user_data=mysqli_fetch_array($que_user_data);
		$userid=$user_data[0];
		$user_join_time=$user_data[6];
		mysqli_query($con,"insert into user_post(user_id,post_txt,post_time,priority) values($userid,'Join Solaris','$user_join_time','Public');");
		mysqli_query($con,"insert into user_status values($userid,'Online')");
		mysqli_query($con,"insert into user_info(user_id) values($userid)");
		
		session_start();
		$_SESSION['suser']=$user;
		header("location:../../s_home/Home.php");
	}
?>
<html>
<head>
...
<?php
			...
			{
				header("location:../s_step2/Secret_Question1.php");
			}
		}
		else
		{
