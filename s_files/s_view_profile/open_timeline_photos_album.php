...
error_reporting(1);
	...
	mysqli_select_db("sdb");
	
	$que_user_info=mysqli_query($con,"select * from users where user_id=$vid");
	$user_data=mysqli_fetch_array($que_user_info);
	$gender=$user_data[4];
	$email=$user_data[2];
	
	$que_post_img=mysqli_query($con,"select * from user_post where post_id=$id");
...
<img src="../../fb_users/<?php echo $gender; ?>/<?php echo $email; ?>/Post/<?php echo $post_img_data[3]; ?>" style="height:500; width:800; ">
</div>
</div>
