...
<?php
	if(isset($_POST['work_sub']))
	{
		$u_job=$_POST['job'];
		$u_edu=$_POST['edu'];
		mysqli_query($con,"update user_info set job='$u_job',school_or_collage='$u_edu' where user_id=$userid;");
	}
	...
	if(isset($_POST['leving_sub']))
	{
		...
		mysqli_query($con,"update user_info set  	current_city='$u_city',hometown='$u_hometown' where user_id=$userid;");
	}
	...
	if(isset($_POST['basic_sub']))
	{
		if($_POST['day']=='Day:' && $_POST['month']=='Month:' && $_POST['year']=='Year:')
		{
			...
			mysqli_query($con,"update user_info set relationship_status='$u_relationship' where user_id=$userid;");
		}
		else
		{
			...
			if(checkdate($month,$day,$year))
			{
				...
				mysqli_query($con,"update user_info set relationship_status='$u_relationship' where user_id=$userid;");
				mysqli_query($con,"update users set Birthday_Date='$u_birthday_date' where user_id=$userid;");
			}
			else
			{
				echo "<script>
	...
	if(isset($_POST['contact_sub']))
	{
		...
		$u_web=$_POST['web'];
		$u_s_id=$_POST['sid'];
		mysqli_query($con,"update user_info set mobile_no='$u_m_no',mobile_no_priority='$u_priority',website='$u_web',solaris_ID='$u_s_id' where user_id=$userid;");
	}
...
<div style="position:absolute;left:31.5%; top:54%; font-weight:bold; z-index:1;"> <a href="Profile.php" style="text-decoration:none; color:#3B59B0;" onMouseOver="on_timeline_txt();" onMouseOut="out_timeline_txt();">  Publications </a> </div>
...
<div style="position:absolute;left:17%; top:139%; height:0.09%; width:66%; background:#CCC; z-index:1">
 ...
 <!--Basic Information--> 
 <?php
	$user_data_query=mysqli_query($con,"select * from users where Email='$user'");
...
<div style="position:absolute;left:53%; top:175%; font-size:18px; color:#89919C;">Solaris ID </div>

<?php
	$s_id=$user_info_data[9];
	if($s_id!="")
	{
?>
		<div style="position:absolute;left:61%;top:175%; color:#3B59A4; font-weight:bold;"> <?php echo $s_id; ?> </div>
...
<?php
	...
	{
?>
		<div style="position:absolute;left:61%;top:175%; color:#3B59A4; font-weight:bold;"> Add Solaris ID </div>
<?php
	}
?> 
...
<div style="position:absolute;left:53%; top:175%; font-size:18px; color:#89919C;">Solaris ID </div>
<div style="position:absolute;left:61%;top:174.4%;"> <input type="text" value="<?php echo $s_id; ?>" name="sid" maxlength="40" style="height:33;width:300;font-size:16px;"> </div>
